...
Ροή δεδομένων: `download → normalize → upsert (ΑΔΑ ως κλειδί) → incremental refresh → paginated/aggregate queries`.
...
| Φόρτωση `/stats` στη μνήμη | έως ~45.000 έγγραφα | καμία (aggregation) |
